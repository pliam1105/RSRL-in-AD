...
def Wang(quantiles: torch.Tensor, beta: float) -> torch.Tensor:
    x = torch.vmap(torch.distributions.Normal(torch.tensor([0.0]), torch.tensor([1.0])).icdf)(quantiles)
...
def run_experiment(risk_measure="neutral", beta=0.0):
    ...
    trainer = OffpolicyTrainer(
        ...
        max_epoch=50, #default: 10, preferred: 20-50? and then it works pretty well
    ...
    for epoch_stat in trainer:
        if(epoch_stat.epoch < trainer.max_epoch -2): # because in the last two epochs it becomes terrible, don't know why
            ...
            # TODO: compute my custom evaluation metrics (over the episodes of the current epoch)
            # from the epoch_stat (and its test returns) and print them, then plot them after the end of training
            ...
            collector.reset()
            collector.collect(n_episode=num_test_episodes, render=1 / 200)
            env.close()
...
if __name__ == "__main__":
    ...
    run_experiment("neutral")
    run_experiment("cvar", 0.25)
    run_experiment("wang", 0.75)
    run_experiment("cpw", 0.71)